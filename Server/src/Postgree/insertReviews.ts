...
export const insertReviews = async (
  ...
  try {
    ...
    const usernames = usersResult.rows.map(
      (user: { username: string }) => user.username
    );
    const movieIds = moviesResult.rows.map((movie: { id: number }) => movie.id);

    console.log("Usernames:", usernames);
    console.log("Movie IDs:", movieIds);

    for (const username of usernames) {
      const numReviews: number = Math.floor(Math.random() * 3) + 1; // Entre 1 e 3 reviews por usuário
